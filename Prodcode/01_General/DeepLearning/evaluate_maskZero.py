...
def pad_labels(true_labels, predicted_labels):
    ...
    Dabei werden die Werte aus der jeweils anderen Liste mit 0 genutzt.
...
def evaluate(eval_file, eval_label, additional_infos=""):
    ...
    model_name = os.path.basename(os.path.normpath(model_load_MaskZero.model_path))
    ...
    with open(eval_file, 'r') as txt_f, open(eval_label, 'r') as csv_f:
        ...
        for i in range(len(txt_zeilen)):
            print(i)
            pred_labels = model_load_MaskZero.predict_and_display(txt_zeilen[i].strip(), False)
